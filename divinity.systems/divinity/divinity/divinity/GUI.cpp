...
void CGUI::Update()
{
	...
	for (auto window : Windows)
	{
		if (window->m_bIsOpen)
		{
			...
			float owo = options::menu.visuals.owo_slider.GetValue() / 500;
			//--
		...
		else
		{
			...
			{
				Render::Clear(1630, 15, 150, 15, Color(71, 206, 229, 255));
		    //  Render::Text((centerW * 2) - 200, 15, Color(255, 255, 255, (MenuAlpha - 1)), Render::Fonts::xd, "Nameless -Beta");
				Render::Textf((centerW * 2) - 285, 15, Color(240, 240, 240, (MenuAlpha - 1)), Render::Fonts::xd, ("divinity.systems -beta | fps: %d "), getfps());
			//	Render::Text((centerW * 2) - 200, 35, Color(65, 105, 225, (MenuAlpha - 1)), Render::Fonts::xd, "");
...
}bool CGUI::DrawWindow(CWindow* window)
{
	...
	float outl_r2 = options::menu.ColorsTab.outl_r2.GetValue();
	float outl_g2 = options::menu.ColorsTab.outl_g2.GetValue();
	float outl_b2 = options::menu.ColorsTab.outl_b2.GetValue();

	float inl_r2 = options::menu.ColorsTab.inl_r2.GetValue();
	float inl_g2 = options::menu.ColorsTab.inl_g2.GetValue();
	float inl_b2 = options::menu.ColorsTab.inl_b2.GetValue();

	float inr_r2 = options::menu.ColorsTab.inr_r2.GetValue();
	float inr_g2 = options::menu.ColorsTab.inr_g2.GetValue();
	float inr_b2 = options::menu.ColorsTab.inr_b2.GetValue();

	float outr_r2 = options::menu.ColorsTab.outr_r2.GetValue();
	float outr_g2 = options::menu.ColorsTab.outr_g2.GetValue();
	float outr_b2 = options::menu.ColorsTab.outr_b2.GetValue();
	...
	if (options::menu.visuals.espwindow.GetState())
	{
		...
		if (options::menu.visuals.OptionsHealth.getindex())
		{
			Render::Clear(28 + 650 + 100 - 4, 374 - 100 + 30, 3, 302, Color(30, 235, 30, MenuAlpha));
		}
		if (options::menu.visuals.OptionsBox.getindex())
		{
			Render::Clear(28 + 650 + 100, 374 - 100 + 30, 202, 302, Color(30, 30, 30, MenuAlpha));
			Render::Clear(29 + 650 + 100, 375 - 100 + 30, 200, 300, Color(options::menu.ColorsTab.BoxCol.GetValue(), MenuAlpha));
		...
		//if (options::menu.visuals.OptionsName())
		{
		...
		//if (options::menu.visuals.OptionsArmor.getindex())
		//{
		//	Render::Clear(15 + 650 + 100 + 124, 93 + 337 - 100 + 30, 30, 24, Color(30, 235, 30, MenuAlpha));
		//}
	}
	...
	if (TabCount) // If there are some tabs
	{
		...
		for (int i = 0; i < TabCount; i++)
		{
			...
			if (window->SelectedTab == tab)
			{
				...
				Render::gradient_verticle(TabArea.left, TabArea.top, TabArea.right, TabArea.bottom + TextSize.bottom - 10, Color(24, 24, 34, 255), Color(24, 24, 34, 255));
				//	Render::gradient_verticle(TabArea.left, TabArea.top + 22, TabArea.right, TabArea.bottom + TextSize.bottom - 30, Color(86, 9, 229, 255), Color(255, 255, 255, 200)); //tab selected line
	...
	Render::Outline(window->m_x - 2, window->m_y - 2, window->m_iWidth + 4, window->m_iHeight + 4, Color(13, 37, 43, 250)); //edges inner
	Render::Outline(window->m_x - 3, window->m_y - 3, window->m_iWidth + 6, window->m_iHeight + 6, Color(13, 37, 43, 250));
	Render::Outline(window->m_x - 4, window->m_y - 4, window->m_iWidth + 8, window->m_iHeight + 8, Color(13, 37, 43, 250));

	Render::Outline(window->m_x - 5, window->m_y - 5, window->m_iWidth + 10, window->m_iHeight + 10, Color(13, 37, 43, 240)); //edges outer
	Render::Outline(window->m_x - 6, window->m_y - 6, window->m_iWidth + 12, window->m_iHeight + 12, Color(13, 37, 43, 240));
	Render::Outline(window->m_x - 7, window->m_y - 7, window->m_iWidth + 14, window->m_iHeight + 14, Color(13, 37, 43, 240));
	static float rainbow;

	if (options::menu.ColorsTab.MenuBar.getindex() == 3) {
...
void CGUI::SaveWindowState(CWindow* window, std::string Filename)
{
	...
	tinyxml2::XMLElement *Root = Doc.NewElement("Divinity");
	...
	{
		MessageBox(NULL, "Unable to save config file. Please try again", "Divinity", MB_OK);
	}
}
