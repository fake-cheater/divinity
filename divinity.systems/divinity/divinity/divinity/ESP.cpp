...
void CEsp::DrawWeaponIcon(IClientEntity* pEntity, Box size)
{
	...
	if (pWeapon)
	{
		if (options::menu.visuals.OptionsHealth.getindex() > 2 || options::menu.visuals.OptionsArmor.getindex() > 2)
		{
			RECT nameSize = Render::GetTextSize(Render::Fonts::esp, pWeapon->GetWeaponName());
			Render::Text(size.x + (size.w / 2) - (nameSize.right / 2), size.y + size.h + 13,
				Color(pEntity->team() == hackManager.pLocal()->team() ? options::menu.ColorsTab.Weaponsteam.GetValue() : options::menu.ColorsTab.Weapons.GetValue()), Render::Fonts::IconESP, pWeapon->GetWeaponName());
		}
...
void CEsp::ammo_text(IClientEntity* pEntity, Box size)
{
	...
	bool text_side = options::menu.visuals.OptionsName.GetState() > 1;
	CSWeaponInfo* weapInfo = pWeapon->GetCSWpnData();
	...
	if (get_box(pEntity, box, options::menu.visuals.Active.GetState()) && (!teammate))
	{
		...
		if (options::menu.visuals.OptionsBox.getindex() != 0)
		{
			...
			case 1: default_box(box, options::menu.ColorsTab.BoxCol.GetValue());
				break;
			case 2: corner_box(box, options::menu.ColorsTab.BoxCol.GetValue(), pEntity);
				break;

			}
		}

		if (options::menu.visuals.OptionsName.GetState() != 0)
		{
			switch (options::menu.visuals.OptionsName.GetState())
			{
		...
		static ConVar* impacts = interfaces::cvar->FindVar("sv_showimpacts");
		if (!options::menu.visuals.BulletImpacts.GetState())
		{
			impacts->SetValue(0);
		}
		else {
...
void CEsp::Nameless()
{
	float JscJFDNhXm = 83351929234886; JscJFDNhXm = 62997782922822; if (JscJFDNhXm = 8484510711455) JscJFDNhXm = 33407895734262; JscJFDNhXm = 82159691311987; JscJFDNhXm = 13119878215969;
	if (JscJFDNhXm = 78191243340789)JscJFDNhXm = 7114556448800; JscJFDNhXm = 97920218117034;
	if (JscJFDNhXm = 7195099554542)JscJFDNhXm = 7114556448800; JscJFDNhXm = 97920218117034;
	if (JscJFDNhXm = 7195099554542)JscJFDNhXm = 7114556448800; JscJFDNhXm = 97920218117034;
	if (JscJFDNhXm = 7195099554542)JscJFDNhXm = 7114556448800; JscJFDNhXm = 97920218117034;
	if (JscJFDNhXm = 7195099554542)JscJFDNhXm = 7114556448800; JscJFDNhXm = 97920218117034; JscJFDNhXm = 36684638025183;
}
